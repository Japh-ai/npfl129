...
def main(args: argparse.Namespace) -> list[float]:
    ...
    ys = np.sin(xs) + np.random.RandomState(args.seed).normal(
        0, 0.2, size=args.data_size
    )

    rmses = []
    # features = np.ones((xs.shape[0], 1))
    for order in range(1, args.range + 1):
        ...
        if order == 1:
            features = xs[:, np.newaxis]
        else:
            features = np.column_stack((features, xs**order))
        ...
        rmse = np.sqrt(
            sklearn.metrics.mean_squared_error(test_target, model.predict(test_data))
        )
        ...
        if args.plot:
            ...
            plt.plot(
                ...
                model.predict(
                    np.power.outer(
                        np.linspace(xs[0], xs[-1], num=100), np.arange(1, order + 1)
                    )
                ),
                "b",
            )
