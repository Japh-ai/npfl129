...
def main(args: argparse.Namespace) -> tuple[tuple[np.ndarray, ...], list[float]]:
    ...
    def forward(inputs):
        ...
        hidden_layer = np.maximum(0, inputs @ weights[0] + biases[0])

        # Compute logits for output layer
        logits = hidden_layer @ weights[1] + biases[1]

        # Softmax activation for output layer
        logits -= np.max(logits, axis=1, keepdims=True)  # For numerical stability
        output_layer = np.exp(logits) / np.sum(np.exp(logits), axis=1, keepdims=True)
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(0, len(shuffled_data), args.batch_size):
            ...
            # Forward pass
            hidden_layer, output_layer = forward(batch_data)

            # Convert target to one-hot encoding
            batch_target_one_hot = np.eye(args.classes)[batch_target]
            # Compute the error (output layer)
            output_error = output_layer - batch_target_one_hot

            # Gradients for weights[1] and biases[1]
            grad_weights_1 = hidden_layer.T @ output_error / args.batch_size
            grad_biases_1 = np.sum(output_error, axis=0) / args.batch_size

            # Backpropagate through ReLU for hidden layer error
            hidden_error = (output_error @ weights[1].T) * (hidden_layer > 0)

            # Gradients for weights[0] and biases[0]
            grad_weights_0 = batch_data.T @ hidden_error / args.batch_size
...
if __name__ == "__main__":
    ...
    print(
        "Learned parameters:",
        *(
            " ".join([" "] + ["{:.2f}".format(w) for w in ws.ravel()[:12]] + ["..."])
            for ws in parameters
        ),
        sep="\n"
    )
